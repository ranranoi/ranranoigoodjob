...
2. 사람인 Open API + 원티드 검색에서 키워드별 공고 수집
3. 근무지(서울/판교/정자)·경력조건으로 1차 필터링
4. Claude API가 후보자 프로필(`candidate_profile.txt`)과 비교해 0~100점 채점
...
2. API Keys 메뉴에서 새 키 생성 후 복사
   - 참고: 채점 1건당 호출 1회, 하루 50~100건 내외면 비용은 매우 적습니다 (보통 월 1달러 미만 수준).
...
| Secret 이름 | 값 |
|---|---|
| `SARAMIN_API_KEY` | 2번에서 발급받은 사람인 access-key |
| `ANTHROPIC_API_KEY` | 3번에서 발급받은 Anthropic API 키 |
| `GMAIL_USER` | `ranplus7@gmail.com` |
| `GMAIL_APP_PASSWORD` | 4번에서 발급받은 16자리 앱 비밀번호 |
| `RECEIVER_EMAIL` | `ranplus7@gmail.com` |
...
- **기업 규모(직원 100명 이상) 자동 필터는 완벽하지 않음**: 사람인/원티드 공개 API 응답에 직원 수 필드가 없어서,
  Claude가 회사명과 알려진 정보를 바탕으로 "추정 채점"합니다. 작은 회사가 가끔 섞여 들어올 수 있으니
